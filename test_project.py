...
def test_number():
#(p, x)
    assert check_num_chaing(25, 5) == 30
    assert check_num_chaing(98, 5) == 98
    assert check_num_chaing(3, -5) == 1
...
def test_color():
#(x)
    assert check_color_chaing('25,+6') == "#2e5dc3"
    assert check_color_chaing('98,-9') == "#c32e2e"
    assert check_color_chaing('66') == "#adadad"
